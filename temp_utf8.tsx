    ...
    <div className="flex h-[calc(100vh-4rem)] w-full overflow-hidden bg-slate-950 text-slate-100 -m-4 md:-m-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 pb-20">
              {filteredItems.map(item => (
                ...
                >
                  ...
                  <div className="mt-3 font-semibold text-blue-400">Γé╣{item.price}</div>
                </Card>
              ))}
            </div>
          </ScrollArea>
                    ...
                    <div key={item.id} className="bg-slate-800 rounded-lg p-3 border border-slate-700 relative group">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="text-sm font-bold text-blue-400">
                          Γé╣{item.menuItem.price * item.quantity}
                        </div>
                      </div>
            ...
            <div className="bg-[#1e1b1a] border-t border-slate-800 flex flex-col font-medium">
              ...
              <div className="px-4 py-3 space-y-1.5 text-[#d1d0c5] tracking-wide">
                <div className="flex justify-between">
                  <span>Sub total</span>
                  <span className="text-white">Γé╣{subtotal.toFixed(0)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Tax(GST 5%)</span>
                  <span className="text-white">Γé╣{tax.toFixed(0)}</span>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center px-4 py-4 text-2xl font-bold tracking-wide text-[#d1d0c5]">
                <span>Total</span>
                <span className="text-white">Γé╣{grandTotal.toFixed(0)}</span>
              </div>
            </div>
          </div>
      ...
      <div className="w-[40%] flex flex-col bg-slate-950">
        <div className="p-4 bg-slate-900 font-bold border-b border-slate-800 text-lg flex items-center justify-between">
          Payment Processing
          <div className="text-sm font-normal text-slate-400">Amount: <span className="font-bold text-white text-lg ml-1">Γé╣{grandTotal.toFixed(2)}</span></div>
        </div>
        ...
        <div className="p-6 flex-1 flex flex-col overflow-y-auto">
            ...
            {paymentMethod === "CASH" && (
              <div className="w-full max-w-sm space-y-6">
                <div className="text-center">
                  ...
                  <div className="text-3xl font-bold text-white">Γé╣{grandTotal.toFixed(2)}</div>
                </div>
                
                <div className="bg-slate-950 border border-slate-800 rounded-lg p-4">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-slate-400">Cash Received:</span>
                    <span className="text-xl font-semibold text-blue-400">Γé╣{numpadValue || "0"}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-xl font-bold ${Number(numpadValue) >= grandTotal ? "text-green-400" : "text-slate-500"}`}>
                      Γé╣{Math.max(0, Number(numpadValue) - grandTotal).toFixed(2)}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-4 gap-2 mb-6">
            ...
            <Button variant="outline" className="h-14 bg-slate-900 border-slate-800 hover:bg-slate-800 hover:text-white" onClick={() => handleNumpad("BACK")}>Γî½</Button>
          ...
          <div className="p-8 flex flex-col items-center text-center">
            ...
            <div className="w-full bg-slate-950 rounded-xl p-4 mb-8 border border-slate-800 text-sm">
              <div className="flex justify-between mb-2">
                ...
                <span className="font-bold">Γé╣{grandTotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between mb-2">
